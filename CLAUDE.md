...
## Verified 0G facts (Wave 1 — confirmed against live docs/repos + a live deploy, mid-2026)
- **SHIPPED = Direct provider path** (not the hosted Router). The credential is a wallet-signed `app-sk-` token (`0g-compute-cli inference get-secret`), sent as `Authorization: Bearer app-sk-…` to the provider's OWN endpoint `POST ${ZG_PROVIDER_URL}/v1/proxy/chat/completions` (OpenAI-compatible; no wallet key at request time). The hosted Router (`router-api.0g.ai/v1`) rejects `app-sk-` chat with 401 (and `/v1/models` is public) — it wants an `sk-` dashboard key. `lib/0g/compute.ts` supports BOTH: `ZG_PROVIDER_URL` set → Direct; unset → Router with an `sk-` key.
- **Verified live (0G mainnet):** provider `0xDB7B4653…` → `https://compute-network-23.integratenetwork.work`, model **`glm-5.1`**, **TeeML** (model runs in-enclave + signs), real reflection in ~8s with `chatId` proof ref. Deployed at https://lumen-snowy-two.vercel.app. Provider discovered read-only via **`@0gfoundation/0g-compute-ts-sdk`** (`createReadOnlyInferenceBroker`) — old `@0glabs/0g-serving-broker` is deprecated/renamed. Discovery done once; `ZG_PROVIDER_URL` baked into env (no runtime SDK/on-chain call).
- **Caveat:** `app-sk-` is locked to ONE provider (no failover) and that host was unreachable from the local dev machine though fine from Vercel. An `sk-` Router key (auto-failover) is more robust — one-line env swap. Private-mode header `X-0G-Provider-Trust-Mode: private` still sent; proof ref via `ZG-Res-Key` header or the completion `id`. Per-request crypto verify (`processResponse`) = Wave 3.
